...
import numpy as np
import time
from toolFunction import convert3DTo1D
...
#       print(observation_size)
    ...
    for e in range(EPISODE_NB):
        ...
        obs1D = convert3DTo1D(obs)
        #printFunctions.printObservation(obs1D)
        obs = obs1D
        obs = np.reshape(obs, [1, obs.size])
        for timeSpend in range(TIME_FOR_ONE_EPISODE):
            env.render()
            if ((timeSpend % 1) == 0 ):
                action = agent.act(obs)
                nextObs, reward, done, info = env.step(action)
                nextObs = convert3DTo1D(nextObs)
                nextObs = np.reshape(nextObs, [1, nextObs.size])
